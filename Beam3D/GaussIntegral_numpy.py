...
class GaussIntegral:
    ...
    def GaussPoints(self):
        ...
        return xi_eta_zeta_weight

    def ShapeFunctions(self, natural_coord):
        # """计算形函数"""
        if self.ctype=="tetra":
            # """计算四面体的形函数"""
            xi=natural_coord[:, 0]
    ...
    def NaturalToPhysical(self, natural_coord):
        # """将自然坐标转换为物理坐标"""
        if self.ctype=="tetra":
            # """四面体单元"""
            N = self.ShapeFunctions(natural_coord)
            x = np.dot(self.region[:, :, 0], N )
            y = np.dot(self.region[:, :, 1], N ) 
            z = np.dot(self.region[:, :, 2], N )
            physical_coord = np.array([x, y, z]).transpose(1, 2, 0)
        if self.ctype=="triangle":
            # """三角形单元"""
            N = self.ShapeFunctions(natural_coord)
            x = np.dot(self.region[:, :, 0], N )  
    ...
    def JacobianDet(self):
        ...
        if self.ctype=="tetra":
            ...
            matrix_tmp=np.array([v1, v2, v3])
            matrix=matrix_tmp.transpose(1, 0, 2)
            J_det = 1/6 * np.abs(np.linalg.det(matrix))
        if self.ctype=="triangle":
            #"""三角形单元"""
            v1=self.region[:, 1] - self.region[:, 0]
            v2=self.region[:, 2] - self.region[:, 0]
            v1v2=np.cross(v1, v2)
            J_det = 1/2* np.linalg.norm(v1v2, axis=1)
        return J_det
    
    def Integral3D(self, f, n: int, region: np.ndarray):
        #"""计算三维体积分值"""
        self.ctype = 'tetra'
        self.n = n
        self.region = region
        ...
        integral_value = np.dot(np.dot(f, weight), J_det)
        return integral_value
    
    def Integral2D(self, f, n: int, region: np.ndarray):
        #"""计算二维面积分值"""
        self.ctype = 'triangle'
        self.n = n
        self.region = region
