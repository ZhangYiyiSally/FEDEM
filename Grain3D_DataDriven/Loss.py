...
class Loss:
    ...
    def loss_function(self, Tetra_coord: torch.Tensor, 
                      Dir_Triangle_coord: torch.Tensor, 
                      Pre_Triangle_coord: torch.Tensor, 
                      Sym_Triangle_coord: torch.Tensor,
                      DrivenData:torch.Tensor) -> torch.Tensor:
        ...
        integral_drivenloss=self.DrivenDataLoss(DrivenData)

        energy_loss = integral_strainenergy - integral_externalwork
        loss = energy_loss + cfg.loss_weight*(integral_boundaryloss + integral_drivenloss)
        ...
        return loss, energy_loss, cfg.loss_weight*integral_boundaryloss, cfg.loss_weight*integral_drivenloss
    ...
    def DrivenDataLoss(self, dri_data: torch.Tensor) -> torch.Tensor:
        xyz_field=dri_data[:, :3]
        u_field=dri_data[:, 3:]
        u_pred = self.GetU(xyz_field)
        mes_loss = nn.MSELoss(reduction='sum')
        driven_loss = mes_loss(u_pred, u_field)
        return driven_loss
