...
if __name__ == '__main__':
    ...
    driven_data=torch.tensor(cfg.DrivenData, dtype=torch.float32).to(dev)
    ...
    for epoch in range(start_epoch, epoch_num):
        ...
        # # 计算损失函数
        loss=Loss(dem)
        loss_value, energy_loss, boundary_loss, driven_loss=loss.loss_function(dom, bc_Dir, bc_Pre, bc_Sym, driven_data)
        # 反向传播
        optimizer.zero_grad()
        loss_value.backward()
        # 梯度范数）
        ...
        # 更新epoch进度条
        tqdm_epoch.update()
        tqdm_epoch.set_postfix({'loss':'{:.5f}'.format(losses[-1]),'eloss':'{:.5f}'.format(energy_loss), 'bloss':'{:.5f}'.format(boundary_loss), 'dloss':'{:.5f}'.format(driven_loss),'lr':'{:.5f}'.format(lr_history[-1])})
